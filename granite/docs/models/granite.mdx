---
title: "Granite 4.0"
description: "Granite 4.0 language models with a hybrid Mamba-2/transformer architecture and Mixture-of-Experts, delivering 70% lower memory use and 2× faster inference."
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="file-code"
    href="/granite/docs/use-cases/all-cookbooks"
  >
...
<br />
<a
  href="https://huggingface.co/blog/ibm-granite/granite-4-nano"
  style={{ color: "#0F62FE" }}
>
...
<table>
  <thead>
    <tr>
      ...
      <th style={{ width: "140px" }}>Architecture Type</th>
      <th style={{ width: "100px" }}>Model Size</th>
      <th>Intended Use</th>
    </tr>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <td>Hybrid, Mixture of Experts</td>
      <td>7B total / 1B activated</td>
      <td>Designed for low latency and local applications, particularly where the task has long prefill or other scenarios where a MoE model is desired</td>
    </tr>
    ...
    <tr>
      ...
      <td>Hybrid, Dense</td>
      <td>3B total</td>
      <td>Designed for low latency and local applications, and as a building block to perform key tasks (like function calling) quickly within agentic workflows</td>
    </tr>
    ...
    <tr>
      ...
      <td>3B total</td>
      <td>Alternative option for users when Mamba2 support is not yet optimized (e.g. llama.cpp, PEFT, etc)</td>
    </tr>
    <tr>
      <td style={{ lineHeight: "1.2" }}>
      ...
      <td>Dense, Hybrid</td>
      <td>1.5B</td>
      <td>Ideal models for edge, on-device, and latency-sensitive use cases</td>
    </tr>
    <tr>
      <td style={{ lineHeight: "1.2" }}>
        Granite-4.0-1B <img src="../images/granite-images/small.png" alt="Granite 1B Hybrid" width="16"/>
      </td>
      <td>Dense, Traditional</td>
      <td>1B</td>
      <td>Alternative option for users when Mamba2 support is not yet optimized (e.g. llama.cpp, PEFT, etc)</td>
    </tr>
    <tr>
      ...
      <td>Dense, Hybrid</td>
      <td>350M</td>
      <td>Similar use cases as Granite-4.0-H-1B, but even smaller and cheaper to run</td>
    </tr>
    <tr>
      <td style={{ lineHeight: "1.2" }}>
...
- When a list of tools is supplied, the chat template automatically formats this list as a system prompt. See [Chat Template Design](/granite/docs/use-cases/prompt-engineering#chat-template-design) for more information about system prompt design in tool-calling scenarios.
...
- Granite 4.0 chat template automatically returns tool calls between `<tool_call>` and `</tool_call>` tags within the assistant turn. Refer to [Chat Template Design](/granite/docs/use-cases/prompt-engineering#chat-template-design) for examples.
- The Granite 4.0 chat template converts content from `tool` role into a user role, where tool responses appear between `<tool_response>` and `</tool_response>` tags. This conversion occurs automatically when using libraries that apply the chat template for you. However, users who build prompts directly in `Jinja` must ensure that tool responses are formatted according to the Granite 4.0 chat template. Please refer to [Chat Template Design](/granite/docs/use-cases/prompt-engineering#chat-template-design) for examples.
...
- The chat template lists documents as part of the `system` turn between `<documents>` and `</documents>` tags. Please refer to [Chat Template Design](/granite/docs/use-cases/prompt-engineering#chat-template-design) for further details.
...
- The chat template in this version handles lists of tools and documents differently. This update is applied automatically when using the `apply_chat_template` function from the transformers library to construct prompts. However, users who craft `Jinja` templates manually must ensure they follow the updated chat template format. Please refer to [Chat Template Design](/granite/docs/use-cases/prompt-engineering#chat-template-design) for more details.
