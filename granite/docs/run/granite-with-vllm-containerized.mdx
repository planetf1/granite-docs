...
_(For stability, you may pin a version tag, e.g., `vllm/vllm-openai:v0.10.2`. Granite models require `vllm` version 0.10.2 and above.)_
...
To run vllm with the Granite 3 models and tool calling, use the additional parameters specified in the vLLM documentation [here](https://docs.vllm.ai/en/v0.10.2/features/tool_calling.html?h=#ibm-granite) as part of the `docker run` command share in Section 2.
